...
import {faUser,faNewspaper, faImage} from "@fortawesome/free-solid-svg-icons"
...
export const Stepper = () => {
	...
	return (
		<>
			<ul className="relative m-0 flex list-none justify-between overflow-visible p-0 transition-[height] duration-200 ease-in-out">
				{steps.map(step => (
					<li
						...
						<span className="flex relative items-center justify-center w-10 h-10 bg-gray-700 rounded-full lg:h-12 lg:w-12 shrink-0">
							<p className="text-white absolute top-14 text-sm font-bold w-max hidden md:block">{step.title}</p>
			...
			{/*step 1*/}
			{countStep===1 && <div className="flex flex-col gap-y-10 w-full transition-all duration-500 ease-in-out pb-20 text-white mt-10">
				<CustomInput lable={'نام کاربری'}/>
				<CustomInput lable={'ایمیل'}/>
				<CustomInput lable={'شماره همراه'}/>
			</div>}

			{/*step 2*/}
			{countStep===2 && <div className="flex flex-col gap-y-10 w-full transition-all duration-500 ease-in-out pb-20 text-white mt-10">
				<div className="">
					<CustomInput lable={'آدرس ملک'}/>
